...
def next_token(prompt: mx.array, model: Mistral, temp: Optional[float] = 0.0):
    def sample(logits):
        if temp == 0:
...
def generate(prompt: str, model: Mistral, tokenizer: Tokenizer, 
             temp: Optional[float] = 0.0, bs: int = 10, limit: int = 512,
             break_tok: Optional[int] = None):
    prompt = mx.array(tokenizer.encode(prompt))
    tokens = []
    for token, ntoks in zip(next_token(prompt, model, temp), range(limit)):
        tokens.append(token)
        if ntoks == 0:
            mx.eval(tokens)
        if break_tok:
